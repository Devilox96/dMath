...
int main() {
    ...
    std::vector <std::vector <dVector3D <double>>> GridCurrentData(Num + Offset * 2);
    std::vector <std::vector <dVector3D <double>>> GridTempData(Num + Offset * 2);
    ...
    std::vector <std::vector <double>> Bottom(Num + Offset * 2);
    
    for (size_t i = 0; i < Num + Offset * 2; i++) {
        GridCurrentData[i].resize(Num + Offset * 2);
        GridTempData[i].resize(Num + Offset * 2);

        Bottom[i].resize(Num + Offset * 2);
    }
    ...
    for (int iTime = 0; iTime < 2000; iTime++) {
        std::vector <double> AlphaX(Num + Offset * 2);
        std::vector <double> AlphaY(Num + Offset * 2);

        for (size_t iY = Offset; iY < Num + Offset; iY++) {
            for (size_t iX = Offset; iX < Num + Offset; iX++) {
        ...
        for (size_t i = Offset; i < Num + Offset; i++) {
            for (size_t j = Offset; j < Num + Offset; j++) {
        ...
        for (size_t i = 1; i < Num + 2; i++) {
            ...
            (*TempData)[i][Num + 2][0] = 2.0 * (*TempData)[i][Num + 1][0] - (*TempData)[i][Num][0];
            (*TempData)[i][Num + 3][0] = (*TempData)[i][Num + 2][0];

            (*TempData)[i][1][1] = 2.0 * (*TempData)[i][2][1] - (*TempData)[i][3][1];
            (*TempData)[i][0][1] = (*TempData)[i][1][1];
            (*TempData)[i][Num + 2][1] = 2.0 * (*TempData)[i][Num + 1][1] - (*TempData)[i][Num][1];
            (*TempData)[i][Num + 3][1] = (*TempData)[i][Num + 2][1];

            (*TempData)[i][1][2] = 2.0 * (*TempData)[i][2][2] - (*TempData)[i][3][2];
            (*TempData)[i][0][2] = (*TempData)[i][1][2];
            (*TempData)[i][Num + 2][2] = 2.0 * (*TempData)[i][Num + 1][2] - (*TempData)[i][Num][2];
            (*TempData)[i][Num + 3][2] = (*TempData)[i][Num + 2][2];
        }

        for (size_t j = 0; j < Num + 2 * Offset; j++) {
            for (int k = 0; k < 3; k++) {
                (*TempData)[1][j][k] = (*TempData)[2][j][k];
                (*TempData)[0][j][k] = (*TempData)[3][j][k];

                (*TempData)[Num + 2][j][k] = (*TempData)[Num + 1][j][k];
                (*TempData)[Num + 3][j][k] = (*TempData)[Num][j][k];
            }
        }
